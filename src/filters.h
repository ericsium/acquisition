...
struct GroupFilterData {
    GroupFilterData(const std::string &type_, double min_, double max_, bool min_filled_, bool max_filled_) :
        type(type_),
        min(min_),
        max(max_),
        min_filled(min_filled_),
...
public:
    ...
    bool checked;
    std::vector<std::pair<ModFilterData, std::vector<ModFilterData> > > group_data;
private:
    Filter *filter_;
};
